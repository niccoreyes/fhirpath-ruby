...
#
# Expected inputs:
#   The current report is produced by script/validate_conformance_corpus.rb
#   with --json and a matching baseline argument. The baseline file is
#   conformance/baselines/<corpus>.json.
#
# Comparison rules:
...
#   - Cora digest comparison is informational; exact baseline SHA is the gate.
...
# Structural validation: report must have expected top-level keys.
...
# Total must match source count (completeness gate).
...
# Defect count must not increase.
...
# Not-run count must not increase (regression: cases that were executable are now skipped).
...
# Strict mode: reject any deviation at all.
