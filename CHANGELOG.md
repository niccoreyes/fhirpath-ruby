...
## [Unreleased]
- Add collection functions `distinct()`, `intersect()`, `exclude()`, `single()`, `sort()`, `subsetOf()`, `supersetOf()`, and `repeat(count)` (issue #92). `distinct()` removes duplicates using `=` equality in first-seen order. `intersect()` and `exclude()` preserve duplicates from the receiver while filtering by the argument collection. `single()` returns a singleton result or empty collection (error only on multi-item input). `sort()` supports default ascending sort and descending sort via unary minus expression (`sort(-$this)`). `subsetOf()` and `supersetOf()` test set membership relationships. `repeat(count)` repeats the receiver collection N times; requires an integer count or raises `TypeError(:expected_integer)`. Closes issue #92.
