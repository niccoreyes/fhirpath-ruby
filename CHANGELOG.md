...
- Harden the external constant boundary with an injectable `ConstantProvider`, predictable `UnknownConstantError` behavior when absent, generic redacted `HostError` messages with `original_cause` retention, and no implicit provider I/O; resolve, terminology, tracing, and cache behavior remain deferred.
