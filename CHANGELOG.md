...
## [Unreleased]
- Parse partial-precision temporal literals (FHIRPath 2.0.0, 5.6): `@2014`, `@2015-02`, `@2015T`, `@2015-02T`, `@2014-01-01T08`, `@T14`, and `@T14:34` are now accepted alongside the full-precision forms, and `precision()` reports the FHIRPath precision code (4/6/8 for date, 2/4/6/9 for time, 4/6/8/10/12/14/17 for dateTime — hour-only literals are normalised to minute precision because FHIR requires hour:minute). String comparison and equality of temporal values with different precision now return an empty collection instead of silently comparing defaulted components, while sub-second precision is compared numerically (`@2012-04-15T15:30:31 = @2012-04-15T15:30:31.0` is `true`). Closes issue #95.
- Add `lowBoundary()`, `highBoundary()`, and `precision()` for temporal values, including default timezone extremes (`+14:00` for the lowest instant, `-12:00` for the highest) and reduced-precision results. `precision()`/`lowBoundary()`/`highBoundary()` on Decimal and Quantity receivers raise `UnsupportedFeatureError` (classified `unsupported`) rather than producing wrong values.
- Add `comparable()` for Quantity values, and extend the bounded UCUM subset with the alternative-symbol atoms `[in_i]` (inch), `[lb_av]` (avoirdupois pound), and `[s]`, implicit digit exponents (`m2`, `cm3`), and FHIRPath calendar-duration units (`1 'month'`, `1 'd'`, `1 'wk'`). Quantity multiplication now composes units (`2.0 'cm' * 2.0 'm'` is `0.040 'm2'`). Quoted calendar-duration keywords apply calendar semantics for Date/DateTime arithmetic, while the UCUM units `mo`/`a` are rejected there with `:unsupported_temporal_unit`, matching the shared suite. Closes issue #94.
- Normalise official-suite `<output>` elements that omit a `type` attribute to the FHIRPath value they contain (Boolean, Integer, Decimal, temporal, Quantity) instead of an opaque `{'$type' => nil, 'value' => ...}` object, and compare vector results on their canonical form: a normalised Decimal or temporal value matches after the JSONL round trip, and a typed expected output (`<output type="date">@1974-01-01</output>`) resolves to the value it denotes. The conformance runner now classifies `UnsupportedFeatureError` as `unsupported` and `HostError` as `host-dependent` in the no-expected-error path too, so an unimplemented capability is no longer reported as a defect.
- Add `script/generate_official_suite_report.rb` and `rake conformance:official_cases` to make the checked-in per-case report artifact reproducible. Official suite (`fhir-test-cases` commit `ebb15f74f95a4731e59099c4244eeed734c9e447`, 935 cases): 518 pass, 374 defect, 43 unsupported, 0 host-dependent, 0 not-run (previously 416/519/0/0/0).
- Fix the conformance comparison so a precision-carrying literal still matches a normalised expected literal after the JSONL round trip (the raw JSON form is tried before the comparable form), which restores the four boundary cases (`LowBoundaryDateMonth`, `HighBoundaryDateMonth`, `LowBoundaryTimeMillisecond`, `HighBoundaryTimeMillisecond`).
- Add `toString()`, `convertsToString()`, and `toQuantity()` for the receivers the official quantity cases exercise (text form `1 'wk'`, string-to-quantity parsing of quoted UCUM units, calendar-duration names, and bare numbers); the wider convertsTo*/toXxx family remains issue #98.
- Map FHIR `Quantity` elements (`Observation.value`, `Observation.valueQuantity`) to `FHIRPath::Quantity` values in the R4 model provider, with `unit`/`code`/`system` metadata and member access, so they compare with quantity literals; an element whose unit is not a supported UCUM atom keeps its raw JSON form rather than being presented as dimensionless. Also run the official R4 corpus with the R4 model (`conformance/official-r4-core.json`), which the suite requires for resource navigation.
- Make `lib/fhirpath/parser.rb` require its own dependencies (`source_span`, `errors`, `types`, `ast`) so the parser can be loaded directly, and move the UCUM atom table to `lib/fhirpath/ucum_units.rb`.
- Import and execute the full official HL7 FHIRPath R4 shared test suite (935 cases from `fhir-test-cases` commit `ebb15f74f95a4731e59099c4244eeed734c9e447`) with XML fixtures resolved to verified JSON counterparts or converted via the new `FHIRXmlConverter` (fail-closed on unsupported structures). 14 XML-only fixtures converted; 840 JSON-fixture cases reclassified; 81 no-fixture cases classified. Closes issue #79; part of #75.
